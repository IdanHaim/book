...
Users can be authenticated in one of two ways. Either using Windows Authentication via user/pass/domain credentials or using OAuth via an API Key^[You can think about this as Windows Authentication and SQL Authentication in SQL Server]. You can configure access to RavenDB by going to the Databases page in the Studio (click on the resources link at the top right corner), then selecting the System Database (on the right of the page near the resources link). Now got to the Settings tab.
...
You can make an API Key by simply going in the studio to the databases page and then press on the Manage Your Server button(on the right of the page)
